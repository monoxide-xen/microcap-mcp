...
**`/IA` (график анализа) в batch работает, `/IC` (рисунок схемы) — нет.**
`/IA Page="Main" Output="x.jpg"` надёжно рисует график анализа. Документированный
парный `/IC Page="<страница>" Output=...` для рисунка схемы здесь не выдал файла
ни при каком имени страницы (`Main`, `Page 1`) и формате (gif/bmp/png/wmf/jpg) —
батч завершается, но файл не появляется. Поэтому драйвер просит только график
анализа; вывода картинки схемы нет.
