...
| `plot` | отдаёт график нетлиста, отрисованный Micro-Cap, как JPEG |
| `plot_schematic` | то же для `.CIR`-схемы (в т.ч. сгенерированной) — картинка-компаньон к `simulate_schematic` |
| `draw_schematic` | рисует саму `.CIR`-схему как SVG родными символами Micro-Cap (сетка, повороты/отражения, подписи) — MC этого в batch не умеет |
| `annotate_schematic` | тот же SVG, но с напряжениями рабочей точки у каждого помеченного узла — размеченная схема |
...
| `generate_transistor_amplifier` | рисует `.CIR` каскада с общим эмиттером на биполярном транзисторе; делитель смещения считается сам под середину питания |
| `generate_emitter_follower` | рисует `.CIR` эмиттерного повторителя (буфер, усиление ≈ 1, низкое выходное сопротивление) |
| `generate_mosfet_amplifier` | рисует `.CIR` каскада с общим истоком на MOSFET; смещение затвора считается из VTO/KP модели под насыщение |
| `generate_differential_pair` | рисует `.CIR` дифференциальной пары на биполярных транзисторах (парафазные выходы OUTP/OUTN, авто-смещение под середину) |
| `generate_current_mirror` | рисует `.CIR` токового зеркала на биполярных транзисторах (Iout ≈ Iref = (Vcc−Vbe)/Rref) |
| `generate_cascode` | рисует `.CIR` каскода (общий эмиттер под общей базой): усиление как у ОЭ, но выше полоса и выходное сопротивление |
...
138 юнит-тестов без Micro-Cap (парсер, `.CIR`, чтение лога, рендер SVG — чистый текст) плюс 30
